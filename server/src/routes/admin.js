...
adminRouter.post('/tournaments',(req,res)=>{ const id=newId('tour'); const {id:_,entrants:__,...body}=req.body; const t={ id, entrants:[], status:'OPEN', rulesVersion:'2026.1', entryFeeCents:0, rebuyCents:0, usesPlayerFunds:false, allowsChipCashout:false, ...body }; try{ assertNoGamblingFlow(t); }catch(e){ return res.status(403).json({error:e.message}); } tournaments.set(id,t); res.json({ok:true,tournament:t}); });
adminRouter.post('/leagues',(req,res)=>{ const id=newId('league'); const {id:_,...body}=req.body; const l={ id, members:[], status:'OPEN', prizeSource:'SPONSOR', ...body }; leagues.set(id,l); res.json({ok:true,league:l}); });
